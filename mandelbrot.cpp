...
// 	v_mov_imm32(p, 8, 0x0);
	...
	v_mul_f32(p, 8, 8, 255); p[0] = floatconv(1.0/float(my/2)/(image_scale*double(mx)/double(my))); p++;
...
void animationZoom(double offset_x, double offset_y, double zoom_step, double zoom, int iternum)
{
	...
	for (int i = 0; i < iternum; i++)
	{
		...
		compute.launch(user_data, {0, 0, 0}, {size_t(mx/256), size_t(my), 1}, {256, 1, 1}, program_code, {program_code, data_cur}, 0, 255, 50);
